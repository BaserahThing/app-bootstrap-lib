...
var AssetManifestGenerator = class {
  ...
  async generate() {
    ...
    if (!fs__namespace.existsSync(this.distDir)) {
  ...
  async scanDirectory(dir, relativePath = "") {
    ...
    for (const item of items) {
      ...
      const relativeFilePath = path__namespace.join(relativePath, item).replace(/\\/g, "/");
  ...
  async processFile(filePath, webPath) {
    ...
    const hasGzip = fs__namespace.existsSync(gzipPath);
    let gzipSize = 0;
    if (hasGzip) {
      gzipSize = fs__namespace.statSync(gzipPath).size;
    }
...
`;
    fs__namespace.writeFileSync(this.outputFile, manifestContent, "utf8");
...
async function generateAssetManifest(distDir = "./dist", outputFile) {
  const defaultOutputFile = path__namespace.join(distDir, "asset-manifest.js");
...
exports.AssetManifestGenerator = AssetManifestGenerator;
exports.generateAssetManifest = generateAssetManifest;
